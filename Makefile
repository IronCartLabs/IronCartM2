...
# ---------------------------------------------------------------------------
# Marketplace package build (Adobe Commerce Marketplace edition).
#
# Wraps `php bin/build-marketplace.php` so contributors don't have to
# remember the argument shape. The script:
#   1. Reads canonical module version from etc/module.xml setup_version.
#   2. Verifies root composer.json + package-marketplace/composer.json
#      extra.module-version both match.
#   3. Stages the OSS module source into
#      package-marketplace/build/staging/.
#   4. Swaps in the Marketplace-shaped composer.json.
#   5. Runs `composer validate --strict --no-check-publish` against the
#      staged tree (if composer is on PATH; CI runs validate independently).
#   6. Produces
#      package-marketplace/build/ironcartlabs-magento-scan-marketplace-<version>.tar.gz
#      ready for Adobe Marketplace submission.
#
# See package-marketplace/README.md for the full design rationale and the
# OSS-vs-Marketplace package comparison.
# ---------------------------------------------------------------------------

.PHONY: build-marketplace
build-marketplace: ## Build the Adobe Marketplace tarball (version-skew checked).
	@command -v php >/dev/null 2>&1 || { echo "ERROR: php not on PATH"; exit 1; }
	php bin/build-marketplace.php

.PHONY: check-marketplace-version
check-marketplace-version: ## Verify module.xml / root composer.json / marketplace composer.json all agree on version.
	@command -v php >/dev/null 2>&1 || { echo "ERROR: php not on PATH"; exit 1; }
	php bin/build-marketplace.php --check-only

.PHONY: clean-marketplace
clean-marketplace: ## Remove the marketplace build artifacts under package-marketplace/build/.
	@if [ -d "package-marketplace/build" ]; then \
		rm -rf package-marketplace/build; \
		echo ">>> removed package-marketplace/build/"; \
	else \
		echo ">>> package-marketplace/build/ already absent"; \
	fi
